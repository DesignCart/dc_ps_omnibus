...
class Dc_Ps_Omnibus extends Module
{
    ...
    public function hookDisplayProductPriceBlock($params){
        try {
            ...
            $current_price = (float) Product::getPriceStatic($id_product, true, $ipa, 2, null, false, true);
            $base_price = (float) Product::getPriceStatic($id_product, true, $ipa, 2, null, false, false);

            // Pokazuj informację tylko wtedy, gdy aktualnie obowiązuje obniżka.
            if ($current_price <= 0 || $base_price <= 0 || $current_price >= ($base_price - 0.001)) {
                return;
            }

            $lowest_price = $this->getLowestPriceForLast30Days($id_product, $id_product_attribute, $current_price, $base_price);
    ...
    private function getLowestPriceForLast30Days($id_product, $id_product_attribute, $current_price, $base_price){
        $file = $this->storage_dir . 'history-' . (int)$id_product . '-' . (int)$id_product_attribute . '.txt';
        if (!file_exists($file)) {
            // Fallback: jeśli brak historii, pokazujemy cenę bazową sprzed obniżki.
            return $base_price > $current_price ? $base_price : false;
        }
        ...
        // Jeżeli mamy tylko jeden wpis i jest równy bieżącej cenie promocyjnej,
        // to najpewniej brak danych historycznych sprzed obniżki -> fallback do bazowej.
        if (count($prices) === 1 && abs($min_price - $current_price) < 0.001 && $base_price > $current_price) {
            return $base_price;
        }
